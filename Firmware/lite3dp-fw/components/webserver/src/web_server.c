...
static const char *WEB_UI_HTML =
    ...
    /* ── Print profile card ── */
    "<div class='card'>"
    "<h2>Print Profile <span id='profwhich' style='font-size:14px;color:#888'></span></h2>"
    ...
    "function slotName(n){return String.fromCharCode(64+n)}"
    /* The active profile is a copy, so its origin is inferred by matching
       it against the saved slots; an edit stops matching and reads as
       "unsaved". */
    "function showWhich(p){"
    "document.getElementById('profwhich').textContent="
    "p.matchingSlot?('— Slot '+slotName(p.matchingSlot)):'— unsaved changes';"
    "renderSlots(p.slotsUsed||[])}"
    ...
    "if(r.status==='ok'){st.textContent='saved';showWhich(r)}"
    "else{st.textContent='Error: '+(r.message||'failed')}}"
    /* Empty slots can still be saved to, but Load is disabled — loading
       one would silently hand back factory defaults. */
    "function renderSlots(used){"
    "used=used||[];"
    "document.getElementById('slots').innerHTML=[1,2,3,4,5,6].map(n=>{"
    "const has=used.indexOf(n)>=0;"
    "return '<span style=\"display:inline-block;margin:0 8px 8px 0\">Slot '+slotName(n)+' '+"
    "'<button class=\"btn\" style=\"padding:4px 10px;font-size:13px\"'+(has?'':' disabled')+"
    "' onclick=\"slot('+n+',\\'load\\')\">Load</button>'+"
    "'<button class=\"btn\" style=\"padding:4px 10px;font-size:13px\" onclick=\"slot('+n+',\\'save\\')\">Save</button>'+"
    "(has?'':'<span style=\"color:#555;font-size:12px\"> empty</span>')+'</span>'"
    "}).join('')}"
    ...
    "if(r.status==='ok'){st.textContent='slot '+slotName(n)+' '+action+'d';"
    "if(action==='load')loadProfile();else showWhich(r)}"
    "else{st.textContent='Error: '+(r.message||'failed')}}"
